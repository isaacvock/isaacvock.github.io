...
abundance. Nucleotide recoding RNA-seq (NR-seq; TimeLapse-seq [@RN88], SLAM-seq [@RN89],
TUC-seq [@RN192], etc.) overcomes these limitations. NR-seq relies on metabolic
...
![**Figure 1:** Overview of a standard NR-seq analysis, adapted from
(Vock and Simon 2023). NR-seq typically involves labeling cells with s4U
and chemically recoding s4U to a cytosine. The data provided by this
...
forth and implemented in a number of bioinformatic tools [@RN155; @RN158; @RN180; @RN181],
...
degradation combined global transcriptional inhibition with RNA-seq [@RN108; @RN106; @RN109]. RNA
levels following a time course of inhibition could be fit to exponential
...
synthesis kinetics. While this approach continues to be widely used, it
...
abundance between inhibition timepoints [@RN113; @RN114]. This introduces additional
...
responses, many of which affect transcript stability [@RN110; @RN112]. This represents a
...
Metabolic labeling with nucleotide analogs offers one such strategy [@RN116; @RN119; @RN115; @RN120].
...
chemistries that conjugated biotin to incorporated labels so that
labeled RNA could be separated from unlabeled via a biotin-streptavidin
pull down [@RN117; @RN119], or immunoprecipitation via antibodies that specifically
recognize the metabolic label [@RN120]. While powerful, these enrichment-based
...
enriched RNAs from nontrivial levels of contamination [@RN121; @RN460]. Labeled and
unlabeled spike-ins have been proposed to quantify and account for some
of these challenges, but these introduce their own experimental
challenges and are unable to account for length biases in the
enrichment [@RN196]. Further innovation was thus required to improve the
...
recoding chemistry (TimeLapse [@RN88], SLAM [@RN89], TUC [@RN192], etc.) to either convert or
...
for this task [@RN88]. Simple kinetic models of the trajectories of these two
...
RNA [@RN156]. In other words, all reads with N or more (N usually being 1 or 2)
...
incorporation rates [@RN159; @RN94]. This latter point is an especially concerning batch
...
from a given feature [@RN180].

A more robust analysis strategy is mixture modeling [@RN88; @RN155]. In this strategy,
assumptions are made about the distributions that best describe the
expected mutational content from labeled and unlabeled RNA. For example,
...
read's U-content is optimal [@RN88; @RN155]. This strategy (or slight variants of it),
...
![**Figure 2:** (From [@RN180]) Simple mutation cutoff
analysis strategies are less accurate than mixture modeling. Comparison
of accuracy of 3 analysis strategies on 3 simulated datasets. Each
column represents analysis for a particular dataset. Datasets differ in
their simulated plabeled (pnew in formalism presented here; 1%, 2.5%,
and 5% mutation rates). Each row represents a particular analysis
strategy. Top row: labeled reads are defined as those with at least 1
(\> 0) mutation. This is the strategy implemented in SLAMDUNK by
default. Middle row: labeled reads are defined as those with at least 2
(\> 1) mutations. Represents another commonly used NR-seq analysis
cutoff. Bottom row: EZbakR analysis with two-component mixture modeling.
Points are colored by density in all plots. $\theta$ = fraction of reads
from labeled RNA. Red dotted lines represent perfect
estimation.](Figure_S1.png)
...
![**Figure 3:** (Adapted from [@RN159]) Two-component binomial
mixture model. The counts of T-to-C mutations, given a number of mutable
nucleotides, can be modeled as coming from a mixture of two binomial
distribution. One of these (that describing the mutation content of
reads from new RNA), has a higher probability of a “success” (mutation)
than the other.](TCBMM.png)
...
up and down over time [@RN193]. Thus, reads from RNA synthesized at different
...
constrain the parameter search space [@RN203]. For example, to fit a hierarchical
...
strongly regularizing priors from sample-wide trends [@RN180]. These priors were
...
with better metrics [@RN208; @RN207]. While simple criteria like the Akaike information
criteria (AIC) are popular due to their implementation ease, more robust
metrics have been developed since the advent of AIC [@RN197; @RN200; @RN198]. For example, in the
...
number of advantages over AIC [@RN199; @RN200].
...
it could still be providing biased estimates [@RN208; @RN207]. Thus, when designing new
NR-seq models, it is advisable to use a multi-pronged approach that weighs several
...
model and compared to the analyzed data [@RN202]. Serious discrepancies between
...
be thoroughly explored and explained. Finally, one should assess
model robustness through simulations from a data generating process more
complicated than that used for model fitting [@RN159]. If the bias introduced by
...
abundances of isoforms [@RN209; @RN212; @RN213; @RN211; @RN210]. I thus developed a similar approach to
estimate isoform-specific NTRs in short read NR-seq data [@RN94].
...
of isoforms with which a read is compatible [@RN214]) and integrates this with
...
isoforms, and to identify NMD sensitive isoforms [@RN94].

Accurate transcript isoform analyses require annotations of expressed
isoforms [@RN215; @RN139]. In our work presenting the isoform-NTR estimation strategy, we
...
annotations [@RN134]. We showed that this approach significantly improved the
...
assembly, but presents its own challenges and shortcomings [@RN134]. Thus, while
isoform-level analyses represent a powerful new paradigm in analyses of
...
phenomenon observed across many distinct datasets [@RN185; @RN176]. Dropout is the
...
unlabeled RNA, referred to as the dropout rate [@RN176]. Thus, rapidly turned
...
![**Figure 4**: Visualizing dropout. A simple model of dropout was
simulated using EZbakR’s EZSimulate() function, where labeled RNA is
lost relative to unlabeled RNA at a rate denoted above each pair of
plots (pdo). grandR implements a visualization strategy that correlates
the NTR rank (1 = smallest) vs. the log-difference in +label and -label
read coverage (dropout). The strength of correlation between these is
related to the rate of dropout. bakR implements a visualization strategy
that allows for direct assessment of the fit of a simple dropout model
on the data. The quantities plotted are similar to those in grandR, just
on different model-relevant scales. EZbakR implements both of these
visualization strategies.](Dropout_plots.png)
...
-   Zeroth-order synthesis kinetics (i.e., transcription is a Poisson
    ...
    in single cells due to phenomena such as transcriptional bursting [@RN217],
    it is likely a decent model of bulk transcriptional behavior [@RN122].
-   First-order degradation (i.e., exponential decay): this means that
    ...
    cytoplasmic mRNA) [@RN216; @RN218; @RN91]. While this assumption is inevitably violated to
...
this setting [@RN146]. The intuition behind how this approach works is that no
...
![**Figure 5:** Comparison of steady-state and non-steady-state (NSS)
analysis strategies. Steady-state data was simulated using EZSimulate()
at 3 different label times (2, 4, and 6 hours). Both of these analysis
...
density. Red dotted line represents perfect estimation.](NSS.png)
...
efforts have included sequencing chromatin-associated RNA [@RN91; @RN220], nuclear RNA [@RN91; @RN220; @RN151; @RN162; @RN177; @RN149],
cytoplasmic RNA [@RN91; @RN220; @RN151; @RN162; @RN177; @RN149], ribosome-associated RNA [@RN91], and membrane-associated RNA [@RN177].
...
compartment [@RN177; @RN180]. The scale factor that relates these quantities is precisely
...
challenge [@RN184; @RN156]. NextGenMap is a non-splice aware aligner that provides an
...
process full-length NR-seq data [@RN244]. While NextGenMap's lack of splice
...
of intron mapping reads) but can still provide reasonable results [@RN245].

For full-length NR-seq data, standard splice-aware aligners like STAR
and HISAT2 were originally preferred [@RN160; @RN221]. Using these tools typically
...
type-specific manner (technically BASAL [@RN466], a recently developed aligner,
...
converted to Cs [@RN179]. By similarly converting Ts in all reads to Cs, reads
...
alignment of NR-seq data [@RN222]. It concluded that neither is strictly superior
...
like rmapalign3N ([Muller et al. 2025](https://pmc.ncbi.nlm.nih.gov/articles/PMC12509878/)). As STAR is more widely used and actively maintained
...
alignment. Towards that end, grandRescue was developed [@RN176]. grandRescue
...
mutational content, making it incompatible with s^6^G NR-seq datasets [@RN36; @RN92].
...
VarScan [@RN461] or BCFtools ([Danecek et al. 2021](https://academic.oup.com/gigascience/article/10/2/giab008/6137722)). Also, while SNP calling has been shown to
significantly impact analyses relying on simple mutation-content cutoffs
for quantifying labeled read abundances, mixture modeling may be more
robust to high background mutation rates and uncalled SNPs. Despite
...
matrix [@RN224]. For each feature analyzed and each sample collected, a count
...
operations and is widely used in a number of bioinformatic libraries [@RN463; @RN462; @RN465; @RN464].
Thus, one could argue that a similar processed data format should be used
for NR-seq.
...
[Apache Arrow project](https://arrow.apache.org/) and its R frontend, which has significantly
increased the scalability of its analyses [@RN94].
...
kinetics of alternative 3'-UTR isoforms [RN240]. This is useful as 3'-UTRs are
well established hubs of post-transcriptional regulation [@RN243; @RN242; @RN241].
...
has been used with other recoding chemistries as well [@RN88; @RN151; @RN177]. While lacking
...
mRNA can be assessed in the same sample with full-length RNA data [@RN180]. In
addition, I developed strategies using full-length RNA NR-seq
data to infer the turnover and synthesis kinetics of individual
transcript isoforms [@RN94]. I worked with Bobby Hogg's lab at the NIH to apply
...
biological processes (see section on extensions at the end) [@RN170]. Thus, users
...
more important consideration [@RN94]. On the other hand, if performing 3'-end
...
sequencing experiments [@RN178]. We have found there to be a significant amount
...
concentrations to avoid cytotoxic effects [@RN127]. Others may require much
...
interested in studying [@RN157]. For human mRNA, this is around 4 hours [@RN120; @RN460]. This
...
![**Figure 6**: Estimate accuracy as a function of NTR. EZbakR’s
SimulateOneRep() function was used to simulate data for 10,000 features,
all of which were given 100, 1000, or 10000 reads. Features had a range
of NTR values, evenly spaced from 0.01 to 0.99. NTRs were estimated with
...
the data reasonably well).](Uncertainties.png)
...
cytotoxic effects, so minimizing this risk is advisable [@RN127; @RN128]. In addition,
...
estimation [@RN157; @RN193]. In addition, grandR implements a strategy that uses multiple
label times to correct for biases that can arise due to gradual ramp up
in s^4^U availability over time [@RN193]. It assumes that the longest label time
...
metabolic label during the chase) [@RN141; @RN143]. Finally, estimating kinetic
...
method variations, etc. than those from pulse-chases [(Vock et al. 2025)](https://www.biorxiv.org/content/10.1101/2025.08.19.671151v1.full).

![**Figure 7:** Two NR-seq labeling strategies. Pulse-labeling refers to
labeling cells for a certain amount of time, after which RNA is
extracted. A pulse-chase refers to labeling cells for (typically) a much
longer period of time, chasing with the regulate nucleotide (e.g.,
uridine), and extracting RNA after the chase. These two designs are
symmetric (the unlabeled RNA in one behaves like the labeled RNA in
another). Despite this, pulse-labels provide a number of technical
advantages to a pulse-chase, and thus should be the default NR-seq
labeling strategy (see text for discussion).](Labeling.png)
...
enrichment of labeled RNA [@RN117; @RN116; @RN468]. While the limitations of this approach
...
cleavage and polyadenylation [@RN196; @RN248]. A major challenge in analyzing these
...
bioinformatically filter out unlabeled RNA contamination [@RN88; @RN170].
...
to continue into productive elongation or prematurely terminate [@RN226; @RN227; @RN225]. To
probe the steady-state levels of paused polymerase, Start-seq was
developed [@RN228]. This approach involves sequencing short, capped RNAs housed
...
recoding, in a method termed Start-TimeLapse-seq, or STL-seq [@RN169].
...
elongation and premature termination (Kamieniarz-Gdula and Proudfoot 2019). Thus, the pause site departure
...
significantly improve the recovery of high mutation content reads [@RN169].
...
While NR-seq experiments typically make use of s^4^U, s^6^G has also shown
to be compatible with these methods [@RN36; @RN92]. This has opened the door for
...
proteomics methods SILAC [@RN231] to make use of dual-labeling for rigorous
normalization of complex sequencing experiments. TILAC involves mixing
s^4^U labeled cells with s^6^G labeled cells. The idea is that these two
...
labeling [@RN232; @RN233]. In addition, s^6^G may not be readily incorporated into the
...
translation initiation [@RN230]. Despite this, these analyses are fundamentally
plagued by kinetic ambiguities that require strong assumptions to
interpret in this way. Thus, a method termed Nascent Ribo-seq was
developed, which combines nucleotide recoding with Ribo-seq [@RN166]. This
...
nucleotide recoding [@RN161; @RN164; @RN165; @RN163; @RN174; @RN171]. This
includes work to further optimize the scalability,
ease-of-implementation, and conversion efficiency of these protocols [@RN187].
...
improve analyses of RNA velocity [@RN172]. This paper also introduced a pipeline for processing scNR-seq data, dynast (GRAND-SLAM is the
...
initial metabolic labeling-based velocity analyses [@RN171; @RN188]. This route is
...
type-specific responses to perturbations [@RN189]. The idea is to feed cells the
...
initial RNA levels) [@RN146]. In addition, single cell data is notably sparse,
...
mapped onto ([Lotfollahi et al. 2022](https://www.nature.com/articles/s41587-021-01001-7); [Lotfollahi et al. 2024](https://pubmed.ncbi.nlm.nih.gov/38729109/)).
...
expression [@RN237]. To dissect how regulation of transcription and RNA decay
underlies these changes, Perturb-seq was recently combined with
nucleotide recoding, in an approach termed PerturbSci-Kinetics [@RN186]. While
...
to identify sites of small molecule binding across the transcriptome [@RN178].
...
isoforms from short read NR-seq [@RN94], long read technologies can open up
investigation of isoform-specific dynamics largely invisible to short
read data (e.g., coupling of TSS and TES usage) [@RN238]. There is currently only
one described attempt at long read NR-seq, in which TUC chemistry has
been combined with PacBio long read sequencing [@RN154].
...
labeled from unlabeled RNA [@RN191; @RN190; @RN249]. These approaches have utilized a range of
...
and degradation [@RN182]. Thus, long read NR-seq and direct RNA sequencing of
