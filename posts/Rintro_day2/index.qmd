...
where things in `<...>` are general placeholders that you would have to edit to get working code. The general features are:
  ...
  - `<GEOM_FUNCTION>`, which in practice often looks like `geom_*()`: The geometry or type of plot (points, bars, lines, tiles, lines, tiles, etc.)
...
1. `inner_join(df1, df2)`: Combines the tables so that only rows with matching values in the join columns of both tables are kept. In the example below, we are joining by the shared column "sample", and "sampleC" is unique to one data frame while "sampleD" and "sampleZ" are unique to the other. Thus, the rows for these three cases are dropped:
...
3. `right_join(df1, df2)`: Same as left join but now the "right table" (`df2` in this case) keeps all of its rows and the "left table" columns will be replaced with NAs when a right table entry is not present in the left table.
