...
1. For reads from new RNA, there is a set probability (call it phigh) that a given mutable nucleotide (e.g., uridines in an s^4^U labeling NR-seq experiment) is mutated. This phigh is the same for all such reads, regardless of the RNA species of origin.
...
1.	Every uridine in a labeled RNA was equally likely to get replaced with s^4^U
2.	Every uridine captured by a sequencing read was equally likely to give rise to a non-s^4^U-related mutation due to sequencing errors, alignment errors, etc.
